...
gulp.task('vendorsJS', () => {
	return gulp
		.src(config.jsVendorSRC, {allowEmpty: true, since: gulp.lastRun('vendorsJS')}) // Only run on changed files.
		.pipe(plumber(errorHandler))
		.pipe(
			babel({
				presets: [
...
gulp.task('customJS', () => {
	return gulp
		.src(config.jsCustomSRC, {allowEmpty: true, since: gulp.lastRun('customJS')}) // Only run on changed files.
		.pipe(plumber(errorHandler))
		.pipe(
			babel({
				presets: [
