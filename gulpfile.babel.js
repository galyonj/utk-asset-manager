...
const lineec = require( 'gulp-line-ending-corrector' ); // Consistent Line Endings for non UNIX systems. Gulp Plugin
// for Line Ending Corrector (A utility that makes sure your
// files have consistent line endings).
const filter = require( 'gulp-filter' ); // Enables you to work on a subset of the original files by filtering them
// using a glob.
const sourcemaps = require( 'gulp-sourcemaps' ); // Maps code in a compressed file (E.g. style.css) back to it’s
// original position in a source file (E.g. structure.scss, which was
// later combined with other css files to generate style.css).
const notify = require( 'gulp-notify' ); // Sends message notification to you.
const browserSync = require( 'browser-sync' ).create(); // Reloads browser and injects CSS. Time-saving synchronized
// browser testing.
...
gulp.task( 'styles', () => {
	return (
		gulp.src( config.styleSRC, { allowEmpty: true } ).pipe( plumber( errorHandler ) ).pipe( sourcemaps.init() ).pipe(
			sass( {
				errLogToConsole: config.errLogToConsole,
				outputStyle: config.outputStyle,
				precision: config.precision,
			} )
		).on( 'error', sass.logError ).pipe( sourcemaps.write( { includeContent: false } ) ).pipe( sourcemaps.init( { loadMaps: true } ) ).pipe( autoprefixer( config.BROWSERS_LIST ) ).pipe( sourcemaps.write( './' ) ).pipe( lineec() ) // Consistent Line Endings for non UNIX systems.
			.pipe( gulp.dest( config.styleDestination ) ).pipe( filter( '**/*.css' ) ) // Filtering stream to only css
			// files.
			.pipe( mmq( { log: true } ) ) // Merge Media Queries only for .min.css version.
			.pipe( browserSync.stream() ) // Reloads style.css if that is enqueued.
			.pipe( rename( { suffix: '.min' } ) ).pipe( minifycss( { maxLineLen: 10 } ) ).pipe( lineec() ) // Consistent
			// Line Endings
			// for non UNIX
			// systems.
			.pipe( gulp.dest( config.styleDestination ) ).pipe( filter( '**/*.css' ) ) // Filtering stream to only css
...
gulp.task( 'stylesRTL', () => {
	return (
		...
		).on( 'error', sass.logError ).pipe( sourcemaps.write( { includeContent: false } ) ).pipe( sourcemaps.init( { loadMaps: true } ) ).pipe( autoprefixer( config.BROWSERS_LIST ) ).pipe( lineec() ) // Consistent Line Endings for non UNIX systems.
...
gulp.task( 'images', () => {
	...
	).pipe( gulp.dest( config.imgDST ) ).pipe(
		notify( {
			message: '\n\n✅  ===> IMAGES — completed!\n',
			onLast: true,
		} )
 ...
 */
gulp.task( 'translate', () => {
	return gulp.src( config.watchPhp ).pipe( sort() ).pipe(
		wpPot( {
 ...
 */
gulp.task( 'zip', () => {
	return gulp.src( config.zipSRC ).pipe( zip( config.zipName ) ).pipe( gulp.dest( config.zipDestination ) );
} );

/**
 * Build task that cleans out the plugins directory and then places
 * a fresh copy
 */
gulp.task( 'build', () => {
	return gulp.src( config.buildSrc ).pipe( gulp.dest( config.buildDest ) );
} );
...
gulp.task(
	...
	gulp.parallel(
		...
		() => {
			...
			// Push our updated project files to the plugins directory on change.
			gulp.watch( config.watchBuildDir, gulp.series( 'build', reload ) );
		}
	)
);
